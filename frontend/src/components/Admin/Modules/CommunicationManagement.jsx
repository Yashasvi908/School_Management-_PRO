...
const CommunicationManagement = () => {
    const [activeTab, setActiveTab] = useState('notices'); // 'notices', 'events', 'direct'
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-card-base p-6 rounded-[2.5rem] border border-border-base shadow-sm">
                <div>
                    <h3 className="text-xl font-black text-text-main uppercase tracking-tighter flex items-center gap-3">
                        <Megaphone className="w-6 h-6 text-indigo-500" /> Communication Hub
                    </h3>
                    <p className="text-text-dim text-[10px] font-bold uppercase tracking-widest mt-1">Institutional Signal Flow & Outreach</p>
                </div>
                <div className="flex gap-2">
                    <button onClick={() => setActiveTab('notices')} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'notices' ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/20 border border-indigo-500' : 'bg-bg-base text-text-dim border border-border-base'}`}>Announcements</button>
                    <button onClick={() => setActiveTab('events')} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'events' ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/20 border border-indigo-500' : 'bg-bg-base text-text-dim border border-border-base'}`}>Operations</button>
                    <button onClick={() => setActiveTab('direct')} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'direct' ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/20 border border-indigo-500' : 'bg-bg-base text-text-dim border border-border-base'}`}>Direct Messages</button>
                </div>
            </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div className="h-48">
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={engagementData}>
                                        ...
                                        <Area type="monotone" dataKey="reach" stroke="var(--primary)" strokeWidth={3} fillOpacity={1} fill="url(#colorReach)" />
                        ...
                        <div className="space-y-4">
                            {[
                                ...
                                { label: 'Upcoming Events', val: '08', sub: 'Next 30 Days', color: 'text-primary' },
                    ...
                    {activeTab === 'notices' && (
                        <div className="space-y-6">
                            <div className="flex justify-end">
                                <button className="px-6 py-3 bg-primary text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-primary/20">+ Post New Notice</button>
                            </div>
                                            ...
                                            <tr key={i} className="border-b border-border-base/30 hover:bg-white/[0.02] transition-colors group">
                                                ...
                                                <td className="px-6 py-5 text-right">
                                                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <button className="p-2 hover:bg-primary/10 hover:text-primary rounded-lg"><Edit3 className="w-4 h-4" /></button>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {events.map((e, i) => (
                                <div key={i} className="glass-card p-8 rounded-[2.5rem] border border-border-base group relative overflow-hidden">
                                    ...
                                    <div className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center text-primary mb-6"><Calendar className="w-6 h-6" /></div>
                                    <h4 className="text-xl font-black text-text-main uppercase tracking-tighter mb-4">{e.title}</h4>
                                    <div className="space-y-3 mb-8">
                                        <p className="text-[10px] font-bold text-text-dim uppercase tracking-widest flex items-center gap-2"><Clock className="w-3 h-3 text-primary" /> {e.date}</p>
                                        <p className="text-[10px] font-bold text-text-dim uppercase tracking-widest flex items-center gap-2"><Megaphone className="w-3 h-3 text-primary" /> {e.location}</p>
                                        <p className="text-[10px] font-bold text-text-dim uppercase tracking-widest flex items-center gap-2"><Users className="w-3 h-3 text-primary" /> {e.attendees}</p>
                                    </div>
                                    <button className="w-full py-4 bg-primary text-white text-[11px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-primary/20 hover:scale-[1.02] transition-all">Manage RSVPs</button>
                                </div>
                            ))}
                            <button className="h-full min-h-[250px] border-2 border-dashed border-border-base rounded-[3rem] flex flex-col items-center justify-center gap-4 text-text-dim hover:border-primary/50 hover:text-primary transition-all group">
